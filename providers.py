...
# JS 原文：eU=[{secret:'...',version:61},...]
#         .map(e => { xored = secret.split("").map((c,i) => c.charCodeAt(0) ^ i%33+9)
#                     key = Buffer.from(xored.join(""), "utf8").toString("hex")
#                     return {secret: Secret.fromHex(key), version} })[0]
_SP_TOTP_SECRETS = [
...
def _fetch_anon_token() -> dict:
    """从 open.spotify.com 获取匿名 access token（动态 TOTP，无需浏览器）。"""
    sp_t = str(uuid.uuid4())
...
def _refresh_anon_tokens() -> tuple:
    """强制重新获取 access_token + client_token，并写入文件缓存。"""
